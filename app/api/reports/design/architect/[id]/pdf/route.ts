...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  ...
  const architectId = params.id;
  if (role === "ARCHITECTO" && architectId !== userId) {
  ...
  const projects = await prisma.designProject.findMany({
    where: {
      architectId,
      OR: [{ createdAt: range }, { updatedAt: range }, { startDate: range }, { dueDate: range }],
    },
  ...
  doc.text(`Arquitecto: ${architect?.name || architect?.email || architectId}`);
  doc.text(`Proyectos en rango: ${projects.length}`);
  doc.text(`Tareas registradas: ${tasks.length}`);
  ...
  sectionTitle(doc, "Proyectos asignados");
  ...
  sectionTitle(doc, "Tareas del arquitecto");
  if (tasks.length === 0) {
  ...
  const filename = `reporte_arquitecto_${architectId.slice(0, 8)}.pdf`;
