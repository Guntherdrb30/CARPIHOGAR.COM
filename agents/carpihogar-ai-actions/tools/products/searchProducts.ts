...
export async function run(input: { q: string }) {
  ...
  if (!q || terms.length === 0) {
    return { success: true, message: "Sin terminos de busqueda", data: [] };
  }
  try {
