...
export async function createArchitectUser(name: string, email: string, password: string) {
  const session = await getServerSession(authOptions);
  if ((session?.user as any)?.role !== 'ADMIN') throw new Error('Not authorized');
  const emailLc = String(email || '').trim().toLowerCase();
  ...
  const user = await prisma.user.upsert({
    where: { email: emailLc },
    update: { name, password: hashed, role: 'ARCHITECTO', alliedStatus: 'NONE' },
    create: { name, email: emailLc, password: hashed, role: 'ARCHITECTO', alliedStatus: 'NONE' },
  });
  revalidatePath('/dashboard/admin/usuarios');
  try {
    if (isEmailEnabled()) {
  ...
  try {
    ...
    if (isEmailEnabled()) {
      ...
      const html = basicTemplate(
        'Verifica tu correo',
        `<p>Se ha creado tu usuario ARQUITECTO. Verifica tu correo para activarlo:</p><p>${verifyUrl}</p>`,
      );
      await sendMail({ to: email, subject: 'Verifica tu correo', html });
    }
  } catch {}
